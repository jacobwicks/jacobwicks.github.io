...
const isNamedBlock = element => {
    ...
    if (firstLine.slice(0, 11).toLowerCase() === 'blocktitle:') {
        const name = firstLine.slice(11, firstLine.length).trim();
        const newInnerHTML = pre.innerHTML.slice(
            indexOfSecondLine,
            pre.innerHTML.length
        );

        console.log(`block named ${name}`, newInnerHTML);
    }
};
...
//curly brackets mean an IIFE
{
    ...
    codes.forEach(code => isNamedBlock(code));
    //each named block, we uh...
    //want to slice off the first line
    //and lose a code line number
    //and get it to line up correctly
    //and record the index
    //and uh, make all the comment functions work right I guess
